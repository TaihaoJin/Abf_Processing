// IncidenceSubsetHandler.cpp: implementation of the CIncidenceSubsetHandler class.
//
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
int CIncidenceSubsetHandler::m_nSN;
CIncidenceSubsetHandler::CIncidenceSubsetHandler()
{
...
vector <structIncidenceSubsetCriteriaNode> CIncidenceSubsetHandler::importSubsetCriteriaNodes()
{
	FILE *fp;
	CString sFileName;
...
int CIncidenceSubsetHandler::getIncidenceSubsetCriterionID(CString sID)
{
	/* 
...
vector <structIncidenceSubsetCriteriaNode> CIncidenceSubsetHandler::readIncidenceSubsetCriteriaFile(FILE *fp)
{
	vector <structIncidenceSubsetCriteriaNode> vstructISCriteriaNodes;
...
bool CIncidenceSubsetHandler::isBurstDelimiter(CString sName)
{
	return sName=="BurstDelimiter(ms)";
}

bool CIncidenceSubsetHandler::isClusterDelimiter(CString sName)
{
	return sName=="ClusterDelimiter(ms)";
}
void CIncidenceSubsetHandler::updateEvl(int NumChannels, int NumLevels,float TimePerPoint,int nNumEvents,
...
void CIncidenceSubsetHandler::buildSubsets()
{
	if(!m_bValidCriteria){
...
structIncidenceSubsetNode CIncidenceSubsetHandler::buildSubset(structIncidenceSubsetCriteriaNode aISCNode)
{
...
vector <int> CIncidenceSubsetHandler::buildSubset(structIncidenceCriterionNode aNode)
{
...
/*
	...
	nID=-1;
*/
bool CIncidenceSubsetHandler::sameIntervals(double dV1, double dV2)
{
...
bool CIncidenceSubsetHandler::invalidDelimiter(double dTc)
{
	return dTc<0.;
}

void CIncidenceSubsetHandler::buildBurstNodes(double dTcb0)
{
...
void CIncidenceSubsetHandler::buildClusterNodes(double dTcb, double dTcc0)
{
...
vector <int> CIncidenceSubsetHandler::getFinalIndexes(int nIncidenceType)
{
...
int CIncidenceSubsetHandler::CompleteSetSize(int nIncidenceType)
{
...
vector <int> CIncidenceSubsetHandler::applySubset(vector <int> aVect, int nIncidenceType, structIncidenceSubsetNode aNode){//nIncidenceType is the type of Incidence pointed by the indexes stored in aVect
	vector <int> vnIndexes;
...
vector <int> CIncidenceSubsetHandler::expandIndexes(vector <int> vnIndexes0, int nTypeA, int nTypeB){//nTypeA is the incidence type of the indexes of the input indexes in vnIndexes0
...
vector <int> CIncidenceSubsetHandler::expandIndex(int index, int nTypeA, int nTypeB){//nTypeA is th incidence type of the input index
	int nType,i,j,n0,n1;
...
vector <int> CIncidenceSubsetHandler::expandIndex(int index, int nType){
	vector <int> vnIndexes;
	int i;
	switch (nType){
	case 0:
...
vector <int> CIncidenceSubsetHandler::shiftIndexes(vector <int> setA, int nShift, int nSize){
	vector <int> setB;
	int n=setA.size();
	int index;
	for(int i=0;i<n;i++){
...
void CIncidenceSubsetHandler::fillUpBaseline(){
	int nLevel;
	float fBaseline=0;
...
vector <structIncidenceSubsetCriteriaNode> CIncidenceSubsetHandler::importSubsetCriteriaNodes(CString sFileName)
{
	FILE* fp;
...
structIncidenceSubsetCollectionNode CIncidenceSubsetHandler::getSebsets(){
	return m_structSubsets;
}

void CIncidenceSubsetHandler::updateISCID()
{
	m_nISCID=m_nSN;
	m_nSN++;
}

int CIncidenceSubsetHandler::getISCID()
{
	return m_nISCID;
}

int CIncidenceSubsetHandler::getEvlID()
{
	return m_nEvlID;
}

int CIncidenceSubsetHandler::getBurstID()
{
	return m_nBurstID;
}

void CIncidenceSubsetHandler::updateBursts(structBurstNodes structBurstNodes)
{
	m_bBurstNode=true;
...
void CIncidenceSubsetHandler::updateClusters(structClusterNodes structClusterNodes)
{
...
int CIncidenceSubsetHandler::getClusterID()
{
	return m_nClusterID;
}

CString CIncidenceSubsetHandler::getISCAsString()
{
	int nSize,i;
	CString sT;
...
CString CIncidenceSubsetHandler::getISCAsString(structIncidenceSubsetCriteriaNode aISCNode)
{
	int nSize,i;
	CString sT;
...
CString CIncidenceSubsetHandler::getICAsString(structIncidenceCriterionNode aICNode)
...
CString CIncidenceSubsetHandler::getEvlFileName(){
	return m_sEvlFileName;
}

AbfNode CIncidenceSubsetHandler::getSubsetsAsTrace(int nKey, AbfNode structRawTrace)//nKey: 0 for level, 1 for amp
{
	int nPolarity=-1;
...
/*
void CIncidenceSubsetHandler::addjustTraceBaseline(AbfNode &aNode){
...
double CIncidenceSubsetHandler::interpolation(double x1, double y1, double x2, double y2, double x)
{
	double k=(y2-y1)/(x2-x1);
	return y1+k*(x-x1);
}
void CIncidenceSubsetHandler::exportAPH_Subset(double dMin, double dMax, AbfNode& abfNode, int nWS, int nIT, double dRatio0){
...
CLinearHistogramDouble CIncidenceSubsetHandler::obtainEventAPH(vector <int> vnIndexes, AbfNode &aNode, double dMin, double dMax)
{
	double dRatio0=1.;
	return obtainEventAPH(vnIndexes, aNode, dMin, dMax, dRatio0);
}
CLinearHistogramDouble CIncidenceSubsetHandler::obtainEventAPH(vector <int> vnIndexes, AbfNode &aNode, double dMin, double dMax, double dRatio0)
{
...
void CIncidenceSubsetHandler::clearSubsets()
{
	m_vstructISCriteriaNodes.clear();
	m_structSubsets.vstructSubsets.clear();
}

void CIncidenceSubsetHandler::appendSubsets(structIncidenceSubsetCriteriaNode aISCNode){
	m_structSubsets.vstructSubsets.push_back(buildSubset(aISCNode));
}
void CIncidenceSubsetHandler::loadIncidenceSubsetCriteriaNodes(){
	m_vstructISCriteriaNodes=importSubsetCriteriaNodes();
	updateISCID();
	m_bValidCriteria=true;
}
void CIncidenceSubsetHandler::loadIncidenceSubsetCriteriaNodes(vector <structIncidenceSubsetCriteriaNode> vstructISCriteriaNodes){
	m_vstructISCriteriaNodes=vstructISCriteriaNodes;
	updateISCID();
	m_bValidCriteria=true;
}
void CIncidenceSubsetHandler::loadIncidenceSubsetCriteriaNodes(CString sFileName){
	m_vstructISCriteriaNodes=importSubsetCriteriaNodes(sFileName);
	m_bValidCriteria=true;
}
void CIncidenceSubsetHandler::appendSubsets(CString sFileName){
...
void CIncidenceSubsetHandler::appendSubsets(vector <structIncidenceSubsetCriteriaNode> vstructISCriteriaNodes){
...
void CIncidenceSubsetHandler::exportDwellTimeHistograms(){
	vector <CLinearHistogramDouble> hists;
...
void CIncidenceSubsetHandler::calDwellTimeRanges(){
	vector <bool> vbFirst;
...
void CIncidenceSubsetHandler::setRisetime(float fRisetime){
	m_bRisetime=true;
	m_nRising=int (fRisetime*1000./m_fTimePerPoint)+1;
}
void CIncidenceSubsetHandler::addjustAmplitude(AbfNode aNode){
...
float* CIncidenceSubsetHandler::getAmplitude(){
	return m_pfAmplitude;
}

vector <structIncidenceSubsetCriteriaNode> CIncidenceSubsetHandler::getISC_AllEvents(){
...
vector <structIncidenceSubsetCriteriaNode> CIncidenceSubsetHandler::getISC_AllBursts(){
	vector <structIncidenceSubsetCriteriaNode> svISC;
	structIncidenceSubsetCriteriaNode ISC;
	ISC.nApplyingIndex=0;
	ISC.nIncidenceType=0;
...
vector <structIncidenceSubsetCriteriaNode> CIncidenceSubsetHandler::getISC_AllClusters(){
	vector <structIncidenceSubsetCriteriaNode> svISC;
	structIncidenceSubsetCriteriaNode ISC;
	ISC.nApplyingIndex=0;
	ISC.nIncidenceType=0;
...
int CIncidenceSubsetHandler::getPropertyType(CString sPropertyName)
{
	int nType=-1;
	/* 
...
int CIncidenceSubsetHandler::getPropertyValue(int nIncidenceType, int nPropertyType, int nIndex, double &dValue, int &nValue, int &nPrintingPrecision)
{
	bool bForward;
...
int CIncidenceSubsetHandler::calLatencyT(int nType){
	if(nType>0){
...
int CIncidenceSubsetHandler::calLatencyN(int nType){
	if(nType>0){
