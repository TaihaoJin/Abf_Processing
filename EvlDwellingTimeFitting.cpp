...
structFittingResultNode CEvlDwellingTimeFitting::FitEvl_Transform(int nFuncKey, structDwellFittingTrialNode aNode, structEvlHistogramNode structEvlHist)
{
	...
	int nNumBins=0,i;
	double dTmin=0.,dX0=0.;
	double dTmax=0.,dX1=0.;
	...
	switch (nFuncKey)
	{
		case 1:
			...
			for(i=0;i<nNumBins;i++)
			{
				dX0=m_structEvlHist.vectFRLogBins.at(nFittingLevel)[i].fLeft;
				dX1=m_structEvlHist.vectFRLogBins.at(nFittingLevel)[i].fRight;
				dTemp=m_structEvlHist.vectFLogHist.at(nFittingLevel)[i];
				dEventsInRange+=dTemp;
				structHistNode aNode;
				aNode.dData=dTemp;
				aNode.dLeft=exp(dX0*dLog10);
				aNode.dRight=exp(dX1*dLog10);
				vHist.push_back(aNode);
			}
			bNode.sBinMethod="Log";
			bNode.nLargestBin=bNode.nLargestLogBin[nFittingLevel];
			break;
		case 2:
			//Not using nFuncKey==2 so far
			nNumBins=m_structEvlHist.vectFLogHist.at(nFittingLevel).size();
			dTmin=m_structEvlHist.vectFRLogBins.at(nFittingLevel)[0].fLeft;
			dTmax=m_structEvlHist.vectFRLogBins.at(nFittingLevel)[nNumBins-1].fRight;
			for(i=0;i<nNumBins;i++)
		...
		case 3:
			...
			for(i=0;i<nNumBins;i++)
			{
				dX0=m_structEvlHist.vectFRLogBins_HB.at(nFittingLevel)[i].fLeft;
				dX1=m_structEvlHist.vectFRLogBins_HB.at(nFittingLevel)[i].fRight;
				dTemp=m_structEvlHist.vectFLogHist_HB.at(nFittingLevel)[i];
				dEventsInRange+=dTemp;
				structHistNode aNode;
				aNode.dData=dTemp;
				aNode.dLeft=exp(dX0*dLog10);
	...
	structExponentialFittingNode aExpTrialNode=MakeExpFittingNode(nFuncKey,aNode);
	aExpTrialNode.fFittingRange.fLeft=dTmin;
	aExpTrialNode.fFittingRange.fRight=dTmax;
	CExponentialFitting aFittingNode(aExpTrialNode, vHist);
	structExponentialFittingNode aExpResultNode=aFittingNode.GetFittedNode();
	MakeEvlDTResultNode(bNode,aExpResultNode);
	double dArea=aExpResultNode.dArea;

	double dMean=0.;
	for(i=0;i<nComponents;i++)
	{
		dMean+=aExpResultNode.dWeights[i]/aExpResultNode.dLambda[i];
	}
	...
	bNode.nNumComponents=nComponents;
	bNode.sFileName=aNode.sEvlFileName;
	bNode.nTrialNumber=aNode.nTrialNumber;
	bNode.dSDev=SDev(bNode.dTimeConstants,bNode.dWeights, nComponents, nFittingLevel, bNode.fFittingRange, nEventsInRange);
	bNode.dSSE=SSE(bNode.dTimeConstants,bNode.dWeights, nComponents, nFittingLevel, bNode.fFittingRange, nEventsInRange);
	bNode.dChiSQ=ChiSQ(bNode.dTimeConstants,bNode.dWeights, nComponents, nFittingLevel, bNode.fFittingRange, nEventsInRange);
	double dMeanDwellTimeH,dMeanDwellTimeHC;
	MeanDwellTimeHist(bNode.dTimeConstants,bNode.dWeights, nComponents, nFittingLevel, bNode.fFittingRange, dMeanDwellTimeH, dMeanDwellTimeHC, nEventsInRange);
...
structFittingResultNode CEvlDwellingTimeFitting::FitEvl_Transform_Original(int nFuncKey, structDwellFittingTrialNode aNode, structEvlHistogramNode structEvlHist)
{
	// nFuncKey 1=log: logarithmic bin, 2=AP: all events fitting:, 3=HB: hibrid bin
	m_structEvlHist=structEvlHist;
	int nComponents=aNode.nNumComponents;
...
{
	//nConfidanceMode==0 --> seting confindence level  without readjust other parameters;
...
structExponentialFittingNode CEvlDwellingTimeFitting::MakeExpFittingNode(int nFuncKey, structDwellFittingTrialNode aNode)
{
	...
	int nComp=aNode.nNumComponents;
	for(int i=0;i<nComp;i++)
	{
		aTrialNode.bFreezeL[i]=aNode.bFreezeT[i];
		aTrialNode.bFreezeW[i]=aNode.bFreezeW[i];
		aTrialNode.dLambda[i]=1./aNode.fTimeConstants[i];
		aTrialNode.dWeights[i]=aNode.fWeights[i];
		aTrialNode.dTimeConstants[i]=aNode.fTimeConstants[i];
	}
	aTrialNode.fFittingRange=aNode.fFittingRange;
	aTrialNode.nNumComponents=nComp;
	return aTrialNode;
}

void CEvlDwellingTimeFitting::MakeEvlDTResultNode(structFittingResultNode &bNode, structExponentialFittingNode aFittingNode)
{
	int nComp=aFittingNode.nNumComponents;
	int i;
	for(i=0;i<nComp;i++)
	{
		bNode.dTimeConstants[i]=1./aFittingNode.dLambda[i];
		bNode.dWeights[i]=aFittingNode.dWeights[i];
	}
	bNode.dFittedArea=aFittingNode.dArea;
	bNode.dLogLikelihood=aFittingNode.dLogLik;
	bNode.fFittingRange=aFittingNode.fFittingRange;
	bNode.nNumComponents=nComp;
	bNode.nNumIterations=aFittingNode.nIterations;
}
